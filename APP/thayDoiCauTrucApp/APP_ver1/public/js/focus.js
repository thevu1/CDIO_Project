...
async function saveFocusSession(completed) {
    ...
    const usedSeconds = Math.max(0, (typeof startSeconds !== 'undefined' ? startSeconds : 0) - (typeof timeLeft !== 'undefined' ? timeLeft : 0));
    ...
    const durationReadable = `${String(mins).padStart(2, '0')}:${String(secs).padStart(2, '0')}`;
    ...
    try {
        const res = await fetch("/api/focus/save", {
            ...
            body: JSON.stringify({
                ...
                focus_duration_readable: durationReadable,
                saved_at: formattedTime,
